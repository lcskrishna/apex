...
if "--cuda_ext" in sys.argv:
    ...
    if torch.utils.cpp_extension.CUDA_HOME is None and (not is_rocm_pytorch):
        ...
        if is_rocm_pytorch:
            import glob
            import shutil
            print ("INFO: hipifying cuda code.")
        ...
        if not is_rocm_pytorch:
            ...
            print ("Skipping SyncBN")
            #sources_syncbn = ['csrc/syncbn.cpp', 'csrc/hip/welford.hip']
            #ext_modules.append(
            #    CUDAExtension(name='syncbn',
            #                  sources=sources_syncbn,
            #                  extra_compile_args= {'cxx' : ['-O3'] + version_dependent_macros,
            #                                       'nvcc' : [],}))
        ...
        if not is_rocm_pytorch:
            ...
            #sources_layernorm = ['csrc/layer_norm_cuda.cpp', 'csrc/hip/layer_norm_hip_kernel.hip']
            #ext_modules.append(
            #    CUDAExtension(name='fused_layer_norm_cuda',
            #                  sources=sources_layernorm,
            #                  extra_compile_args={'cxx' : ['-O3'] + version_dependent_macros,
